...
var View_Model = function () {
    ...
    self.load_settings = function () {
        ...
        if (settings.divideX > $(window).width() - 10) {
            self.settings.divideX($(window).width() / 2);
        }
    };
    ...
    self.load_contributors = function () {
        if (self.contributors_loaded === false) {
            ...
            $.get(
                'https://api.github.com/repos/websiteduck/Run-PHP-Code/contributors', function (data) {
    ...
    self.editor.renderer.on(
        'themeLoaded', function () {
            $(window).resize();
            $.each(
                self.themes.light, function (i, theme) {
                    if (theme.theme === self.settings.theme()) { self.light_theme(true);
                    }
                }
            );
            $.each(
                self.themes.dark, function (i, theme) {
    ...
    $(window).resize(
        function () {
            self.resize_window($(this).width(), $(this).height());
        }
    ).resize();
};
    ...
    function () {
        ...
        $('.drop').hover(
            function () {
                ...
                var self = this;
                hide_menu_timeout[$(this).uniqueId().attr('id')] = setTimeout(
                    function () {
                        $('> div', self).stop(true,true,true).slideUp(100); }, 300
                );
            }
        );
        ...
        $('.subdrop').hover(
            function () {
                ...
                hide_menu_timeout[$(this).uniqueId().attr('id')] = setTimeout(
                    function () {
                        $('> div', self).css('left', '-9999px'); }, 300
                );
            }
        );

        $('#title_bar').on(
            'mouseenter', '.drop > div > div, .subdrop > div > div, .drop label, .button', function () {
        ...
        $('#title_bar').on(
            'mouseleave', '.drop > div > div, .subdrop > div > div, .drop label, .button', function () {
        ...
        $('#title_bar .drop, #title_bar button').click(
            function () {
                setTimeout(
                    function () {
                        vm.editor.focus();}, 50
